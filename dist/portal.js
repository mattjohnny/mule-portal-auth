...
export async function redeemSso(opts, ssoToken) {
    ...
    if (!resp.ok)
        throw new PortalError("That sign-in link is invalid or has expired.");
...
export async function fetchContext(opts, email) {
    let resp;
    try {
        resp = await fetch(`${opts.portalUrl}/api/context?email=${encodeURIComponent(email)}`, { headers: { "x-portal-key": opts.sharedKey } });
    }
    catch {
    ...
    if (!resp.ok)
        throw new PortalError("Portal rejected the context request.");
...
function fallbackContext(email, name, role) {
