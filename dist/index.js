...
export function createPortalAuth(config) {
    ...
    const portal = { portalUrl, sharedKey, appName };
    db.exec(`
  ...
  `);
    // --- Local session store --------------------------------------------------
    function startLocalSession(ctx) {
        const token = crypto.randomBytes(32).toString("hex");
        const now = Date.now();
        db.prepare(`INSERT INTO portal_sessions (token, email, name, context, created_at, expires_at, last_validated)
       VALUES (?, ?, ?, ?, ?, ?, ?)`).run(token, ctx.email, ctx.name, JSON.stringify(ctx), now, now + sessionTtlMs, now);
    ...
    function getRow(token) {
        const row = db
            .prepare("SELECT token, email, name, context, expires_at, last_validated FROM portal_sessions WHERE token = ?")
            .get(token);
        if (!row)
            return null;
    ...
    function saveContext(token, ctx) {
        db.prepare("UPDATE portal_sessions SET context = ?, name = ?, last_validated = ? WHERE token = ?").run(JSON.stringify(ctx), ctx.name, Date.now(), token);
    }
    ...
    async function signInWithGoogle(idToken) {
        ...
        if (adminEmails.has(email))
            return startLocalSession(bootstrapAdminContext(email, name));
        let ctx;
        try {
    ...
    async function revalidateIfStale(session) {
        ...
        if (Date.now() - row.last_validated < revalidateMs)
